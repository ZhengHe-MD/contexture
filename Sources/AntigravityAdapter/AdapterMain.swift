...
// testable via `@testable import` without a script-mode main executing
// (and here, blocking on a real stdin read) the moment the test binary
// starts.
@main
enum AntigravityAdapterMain {
    static func main() {
        ...
        // neutral result is a specific JSON shape (`{}`), not "write
        // nothing", so this always writes exactly one JSON object.
